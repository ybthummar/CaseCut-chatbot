...
import { 
  SendHorizontal, Menu, LogOut, User, Plus, 
  MessageSquare, ChevronDown, Scale, Briefcase, GraduationCap,
  Loader2, BookOpen, X, ThumbsUp, ThumbsDown, Filter
} from 'lucide-react'
  ...
  const sendQuery = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/query`, {
        ...
        body: JSON.stringify({ query: input, role, topic, k: 5 })
      })
      ...
      if (user) {
        setTimeout(() => saveChatToHistory(updatedMessages), 500)
      }
    } catch (err) {
            ...
            >
              ...
              <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-semibold text-white">CaseCut AI</span>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-3 py-2">
                ...
                {chatHistory.map((chat) => (
                  <button
                    key={chat.id}
                    className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left text-[#a0a0a5] hover:bg-white/5 hover:text-white transition-all duration-150 mb-0.5"
                  >
          ...
          {messages.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-4">
              <div className="max-w-2xl w-full text-center">
                ...
                <p className="text-[#8a8a8f] text-base mb-2">
                  AI-powered legal research for Indian courts and judgments.
                </p>
                {!user && (
                  <p className="text-xs text-amber-400/80 bg-amber-400/5 border border-amber-400/10 px-4 py-2 rounded-lg inline-block mb-8">
                    Chat history is not saved. Sign in to save your research.
                  </p>
                )}
                        ...
                        {msg.cases.map((c, j) => (
                          <details key={j} className="text-xs group">
                            <summary className="cursor-pointer text-[#4da5fc] hover:text-[#6ab8ff] transition-colors font-medium">
                              Case {j + 1} {c.court && `(${c.court})`} {c.ipc_sections?.length > 0 && `- IPC ${c.ipc_sections.slice(0,2).join(', ')}`}
                            </summary>
