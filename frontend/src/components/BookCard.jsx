  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h4 className={`text-sm font-semibold ${meta.textPrimary} leading-snug line-clamp-2 group-hover:text-purple-700 transition-colors`}>
          {book.title}
        </h4>
        <p className={`text-xs ${meta.textSecondary} mt-0.5 flex items-center gap-1 truncate`}>
          <User className="size-3 shrink-0" />
          {authors}
        </p>
        {book.description && (
          <p className={`text-xs ${meta.textSecondary} mt-1 line-clamp-2 leading-relaxed`}>
            {book.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          {book.pageCount > 0 && (
            <span className={`text-[10px] ${meta.textSecondary}`}>{book.pageCount} pages</span>
          )}
          {book.publishedDate && (
            <span className={`text-[10px] ${meta.textSecondary}`}>{book.publishedDate.slice(0, 4)}</span>
          )}
          <ExternalLink className={`size-3 ${meta.textSecondary} group-hover:text-purple-500 ml-auto transition-colors`} />
        </div>
      </div>
    </motion.a>
  )
